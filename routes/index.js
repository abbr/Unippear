...
// Get combined.js
router.get('*/combined.js', async function (req, res) {
    ...
    let files = await recursive(path.resolve(jsPath))
    if (!files) {
...
router.get(/^(.*)\/(index\.js)?$/, async function (req, res) {
    ...
    if (!router.combineJs) {
        var jsPath = path.join(__dirname, '../', router.publicFolderNm, '/assets', thisFileUrlPath, '/js/')
        try {
            jsFiles = (await recursive(jsPath) || []).map(function (v) {
