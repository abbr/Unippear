...
* If total size of JS files is large, consider using [AMD](http://en.wikipedia.org/wiki/Asynchronous_module_definition). If so, only put AMD engine such as RequireJS and the bootstrap code in */public/assets/js*. Save AMD modules in another folder under */public/assets*, say */public/assets/modules*.
